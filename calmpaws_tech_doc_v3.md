...
8. [数据持久化架构](#8-数据持久化架构)
9. [测试数据分类管理规范](#9-测试数据分类管理规范)
10. [上线前必改清单](#10-上线前必改清单)
...
> **⚠️ 重要架构说明（v2.0 起）：**  
> 本项目已从「硬件端规则算法（ESP32 C++ 固件）」迁移至「服务器端 AI + 物理算法」双轨方案。  
> 下表中标注了每个行为的**当前判断方式**与**历史方式**，避免混淆。

#### 各行为当前判断方式总览

| 行为类型 | **当前判断方式（v2.0 服务器架构）** | 历史方式（v1.0 ESP32 固件） |
|---------|----------------------------------|--------------------------|
| **应激（Stress）** | `algorithm.py` 物理算法：G值1.2–2.2g，相邻差>0.3，满足2点以上 | `peak_g > 1.5 && duration < 0.5s` |
| **发抖（Shiver）** | **AI 模型**（TFLite 标签 = `shivering`）→ `shiv_c+1, shiv_d+=5s` | `freq > 10Hz && amplitude < 0.3g` |
| **踱步（Pacing）** | **AI 模型**（TFLite 标签 = `pacing`）→ `pace_d+=5s` | `0.5Hz < freq < 2Hz && amplitude > 0.5g` |
| **玩耍（Play）** | **AI 模型**（TFLite 标签 = `playing`）→ `play_d+=5s` | `multi_axis_high && irregular` |
| **翻身（Roll）** | `algorithm.py` 物理算法：Z轴从>0.6g → <-0.6g | Z轴极性反转（相同逻辑） |
| **平静（Calm）** | **AI 模型**（TFLite 标签 = `calm`，兜底） | 不满足以上条件 |
...
发抖检测完全由 Edge Impulse TFLite 模型承担，不使用硬件侧规则判断：

```
ESP32 → 100Hz 原始 XYZ
       ↓ 降采样 100Hz → 40Hz（取偶数行）
       ↓ 取最近 120 行（3秒窗口 @40Hz）
       ↓ 每轴提取13个DSP特征（RMS / 偏度 / 峰度 / Welch FFT bin 1–8）
       ↓ TFLite模型推理 → 4类概率 [calm, pacing, playing, shivering]
       ↓ 置信度 < 55% → 保留上一标签（防噪声跳变）
       ↓ 标签 = shivering → shiv_c+1, shiv_d+=5s
```

**关键特征：高频小幅振动在 FFT 高频 bin（bin 5–8）能量显著**  
- 发抖频率范围：**4–16 Hz**（生理性颤抖、恐惧颤抖、疼痛颤抖）  
- RMS 特征：低~中（接近背景1g，无大幅位移）  
- 与踱步区分：踱步 FFT 集中在低频 bin（bin 1–2，0.5–2 Hz）

**发抖信号物理特征参考文献：**

| 论文 | 作者/来源 | 关联内容 |
|------|----------|---------|
| *Postural tremor analysis in dogs and cats using accelerometry and video-based applications* | ResearchGate, 2024 | 直接验证：加速度计测量犬类姿势性震颤的频率和g值范围，为 >10Hz 阈值提供实测依据 |
| *Primary orthostatic tremor and orthostatic tremor-plus in dogs: 60 cases (2003–2020)* | J Vet Intern Med, 2022 | 记录犬类站立时高频震颤13–18 Hz，支持高频区间判断 |
| *Recent Advances in Tremors in Dogs and Cats* | ResearchGate, 2024 | 综述犬类震颤类型（生理性/病理性），频率范围4–16 Hz |
| *Generalized Tremors in Dogs: 198 Cases (2003–2023)* | PMC11951301, 2025 | 大样本震颤研究，描述发抖的幅度、频率与临床分类 |
| **Edge Impulse petoteco #968791** | 本项目训练集 | 直接依据：用实际发抖/踱步/玩耍/平静数据训练，准确率90.31% |

---

#### 应激检测算法详细说明（物理算法，保留规则）

**当前算法（v2.0）：`algorithm.py` 宏颤抖规则（对齐旧 C++ 固件逻辑）**

```python
# algorithm.py 关键逻辑（每5秒窗口）
macro_count = sum(
    1 for i in range(1, len(g_vals))
    if 1.2 <= g_vals[i] <= 2.2         # G值在中等力度范围
    and abs(g_vals[i] - g_vals[i-1]) > 0.3  # 相邻采样差 > 0.3g（节律抖动）
)
if macro_count >= 2 and max_g < 4.0:   # 满足2个点 且 非剧烈碰撞
    state.str_c += 1
    state.str_d += interval_sec
```

**历史版本（v1.0）：ESP32 固件规则（已废弃，仅供对照）**

```c
// Arduino 伪代码（每100ms采样一次）—— 仅 v1.0 使用，v2.0 已废弃
void loop() {
  ...
  else if (freq > 10 && amplitude < 0.3) {  // ⚠️ v2.0 已废弃：发抖改由AI识别
    shiver_count++;
    shiver_duration += shiver_length;
  }
  else if (freq > 0.5 && freq < 2.0 && amplitude > 0.5) {  // ⚠️ v2.0 已废弃：踱步改由AI识别
    pace_duration += 0.1;
  }
  else if (multi_axis_active && irregular) {  // ⚠️ v2.0 已废弃：玩耍改由AI识别
    play_duration += 0.1;
    if (detected_roll) roll_count++;
  }
...
// 注：v2.0 架构中，ESP32 只负责上传原始 XYZ 数据，所有行为分类在服务器完成
```
...
> **背景：** 系统中存在两类性质不同的"测试数据"，必须明确区分，避免上线时改错或漏改。

### 9.1 第一类：纯测试用数据（🔴 上线前必须改回生产值）

**定义：** 为方便开发期快速看到效果，人为调低阈值，**生产环境绝不能用**，上线前必须改回。

**判断标准：**
- 生产值和测试值不一样
- 测试值是为了「缩短等待时间」「降低触发门槛」而人为设置的
- 不代表真实的业务逻辑或实证数据

| # | 文件 | 变量 | 当前值（测试用） | 生产值 | 改动原因说明 |
|---|------|------|--------------|--------|------------|
| 1 | `pet_health_provider.dart` | `kShiverThreshold` | `30` 秒 | `180` 秒（3分钟） | 测试只需等30秒看触发效果 |
| 2 | `pet_health_provider.dart` | `kStressFreqThreshold` | `3` 次 | `10` 次 | 测试时3次就触发，便于快速验证 |
| 3 | `pet_health_provider.dart` | `kStressFreqCooldownMinutes` | `15` 分钟 | `60` 分钟 | 测试时15分钟内可反复触发 |
| 4 | `pet_health_provider.dart` | `kPacingLongThreshold` | `120` 秒 | `1800` 秒（30分钟） | 测试只需踱步2分钟就触发 |
| 5 | `pet_health_provider.dart` | `kLethargyThreshold` | `60` 秒 | `10800` 秒（3小时） | 测试静止1分钟就触发昏睡预警 |
| 6 | `pet_health_provider.dart` | `kNightStressThreshold` | `2` 次 | `5` 次 | 测试夜间2次应激就触发 |
| 7 | `mock_ble_service.dart` | 整个文件 | MockBleService | flutter_blue_plus 真实实现 | 测试时用模拟数据，生产需真实蓝牙 |

**⚠️ 操作规范：**
- 代码提交时，所有第一类测试数据的变量必须在**常量定义处**标注注释：`// 测试值：X | 生产值：Y`
- 上线前用「上线前必改清单」做逐项核查
- **禁止**以「测试值」提交生产 release 包

---

### 9.2 第二类：暂定数据（🟡 测试和生产都用，但将来可能随实证数据调整）

**定义：** 目前测试和生产使用**同一个值**，不是人为凑的，是当前能获得的最合理值，但尚无充分实证，**上线不用改，但未来迭代时应基于真实数据重新验证**。

**判断标准：**
- 测试值 == 生产值（不需要在上线时修改）
- 是从文献、经验或工程判断推导的合理值
- 「叫测试数据」只是因为没有实证数据，不是因为它不能用于生产

| # | 位置 | 当前值（测试=生产） | 设定依据 | 未来验证方向 |
|---|------|-----------------|---------|------------|
| A | `anxietyScore`：`strC × 20` | 20分/次 | 工程估算：应激是最强信号 | 收集真实用户数据后用回归分析标定 |
| B | `anxietyScore`：`paceD × 3` | 3分/秒 | 工程估算：踱步强度中等 | 同上 |
| C | `anxietyScore`：`shivD × 6` | 6分/秒 | 工程估算：发抖比踱步严重 | 同上 |
| D | `anxietyScore`：`strD × 2` | 2分/秒 | 工程估算：应激持续时长辅助加分 | 同上 |
| E | `kStateConfirmPackets = 2` | 2包（10秒确认） | 工程经验：避免单包噪声；2包平衡响应速度与稳定性 | 根据硬件真实噪声水平可调为1–3 |
| F | `kAnxietyWeights = [0.5, 0.3, 0.15, 0.05]` | 见A方案 | 工程经验：指数衰减，最新包权重最高 | 可A/B测试不同权重对用户感知的影响 |
| G | `UNCERTAIN_THRESHOLD = 0.55` | 55% | Edge Impulse 默认值 + 实测可用 | 收集边界样本后可微调 |
| H | 应激物理算法：G值1.2–2.2，相邻差>0.3 | — | 对齐旧 ESP32 固件逻辑（已有硬件实测） | 等硬件团队在新 ESP32 实测后重新标定 |
| I | 发抖信号特征：4–16 Hz 高频区间 | — | 文献支持（McConnell & Garosi, Packer et al.） | 等真实发抖样本采集后重新验证 |

**✅ 操作规范：**
- 第二类数据**不需要**加「上线前必改」标记
- 代码注释写：`// 暂定值，待实证数据后调整 | 当前合理范围：[X–Y]`
- 每季度产品迭代时，回顾「暂定数据」列表，结合真实用户行为数据评估是否需要调整
- 调整需记录变更原因和数据依据
...
| 特征 | 第一类（纯测试用） | 第二类（暂定，测试=生产） |
|------|-----------------|----------------------|
| 测试值 ≠ 生产值 | ✅ 是 | ❌ 否（同值） |
| 上线前必须改 | ✅ 是 | ❌ 否 |
| 代表真实业务逻辑 | ❌ 否（人为调低） | ✅ 是（当前最优估计） |
| 有无实证数据 | 不适用 | 无实证（但合理） |
| 代码注释标记 | `// 测试值：X \| 生产值：Y` | `// 暂定值，待实证后调整` |
| 迭代处理 | 上线前清单逐一核查 | 季度迭代时评估是否更新 |
...
### 🟡 建议评估（上线前 + 季度迭代）— 第二类暂定数据

> 以下参数测试值=生产值，上线时**不需要改**，但属于「暂定值」，未来应根据实证数据评估调整。

| # | 说明 | 对应分类 |
|---|------|---------|
| 8 | `anxietyScore` 权重（strC×20, paceD×3, shivD×6）需数据科学家用真实用户数据验证后调整 | 第二类暂定 |
| 9 | `kStateConfirmPackets = 2`（B方案包数）可根据硬件真实噪声水平调整为1–3 | 第二类暂定 |
| 10 | `kAnxietyWeights`（A方案权重）可根据用户体验A/B测试结果调整 | 第二类暂定 |
| 11 | 每日总结内容目前为估算值，接入真实云数据后需重写 `_triggerDailySummary` | 第二类暂定 |
| 12 | Demo 历史数据（`_seedHistoricalSessions`）在真实用户首次启动时仍会显示 | 第一类（上线前需处理） |
...
// ── A方案：加权平均焦虑分 ─────────────────────────────────────────────────
// 暂定值，待实证数据后调整 | 指数衰减，最新包权重最高
const List<double> kAnxietyWeights = [0.5, 0.3, 0.15, 0.05]; // 最新→最旧

// ── B方案：状态确认包数 ───────────────────────────────────────────────────
// 暂定值，待实证数据后调整 | 合理范围：1–3（硬件噪声越大建议越高）
const int kStateConfirmPackets = 2;        // 连续2包（10秒）才切换状态

// ── 预警阈值 ─────────────────────────────────────────────────────────────
// ⚠️ 第一类测试数据（上线前必须改回生产值！）
const int kShiverThreshold          = 30;   // 测试值：30s  | 生产值：180s（3分钟）
const int kStressFreqThreshold      = 3;    // 测试值：3次  | 生产值：10次
const int kStressFreqCooldownMinutes = 15;  // 测试值：15min | 生产值：60min
const int kPacingLongThreshold      = 120;  // 测试值：120s | 生产值：1800s（30分钟）
const int kLethargyThreshold        = 60;   // 测试值：60s  | 生产值：10800s（3小时）
const int kNightStressThreshold     = 2;    // 测试值：2次  | 生产值：5次
