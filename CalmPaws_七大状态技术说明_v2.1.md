...
**v2.1更新说明（基于代码核查）：**
- **修复架构图ESP32层错误描述**：V7.2固件不做任何物理计算，只上传原始XYZ；micro_tremor/macro_tremor/roll_c均在服务器algorithm.py中计算
...
│  ⚠️ 不做任何物理计算，只存原始数据                        │
│                                                         │
│  输出：原始XYZ（每次500行，5秒@100Hz）+ 心跳包            │
│  ├── 实时模式：每5秒上传一次（APP在线时）                 │
│  └── 后台模式：每60秒上传一次（APP离线时）                │
└─────────────────────┬───────────────────────────────────┘
                      │ HTTP POST /api/upload（原始XYZ only）
┌─────────────────────▼───────────────────────────────────┐
│                    服务器层                              │
│                                                         │
│  inference.py（AI推理）：                                │
...
│                                                         │
│  algorithm.py（物理算法，与AI推理并行）：                 │
│  ├── stressed：macro_tremor≥2(G:1.2~2.2,dG>0.3)        │
│  │           ⚠️ 与步行G值有重叠，靠组合条件区分           │
│  ├── shivering：micro_tremor≥3(G:0.8~1.2,dG:0.03~0.2)  │
│  └── roll_c：Z轴首尾翻转检测（z首>0.6 AND z尾<-0.6）    │
│                                                         │
...
| v2.1 | 2025年7月 | **修复架构图ESP32层错误**（V7.2不做物理计算，只传原始XYZ）；修复焦虑分架构图旧值；修复应激频繁冷却时间内部矛盾；修复E2触发条件（删除不存在的str_c判断）；新增kSleepEntryThreshold参数；新增服务器睡眠计时同步B方案说明；新增加权平均焦虑分A方案；新增kMinPlaySeconds参数；新增kDebugMode机制说明；更新固件版本至V7.2 |
